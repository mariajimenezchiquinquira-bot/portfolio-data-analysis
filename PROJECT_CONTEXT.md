...
### Vehicle Contract Automation (n8n) — botón "How It Works" con documentación técnica (sesión posterior)
...
- Se creó documentación técnica en PDF (una página tipo "ficha de proceso", diseño sobrio estilo documentación interna de Automation Engineering — no usa la paleta/estilo del portafolio a propósito): explica el problema, descripción, propósito, arquitectura, flujo nodo por nodo (10 nodos de n8n), la decisión técnica de usar Google Gemini como modelo de IA, y resultados/potencial impacto de negocio. Es un Learning Project/PoC, por lo que el documento NO inventa métricas reales ni menciona una implementación real en producción.
- El PDF final (versión en inglés, que es la que quedó enlazada) está en `public/docs/vehicle-contract-automation-technical-documentation.pdf`.
- El botón "How It Works" en `src/routes/index.tsx` apunta a `/docs/vehicle-contract-automation-technical-documentation.pdf` (ruta relativa servida como archivo estático).
- Nota: este mismo cambio (documentación técnica + botón "How It Works") también se hizo en `portfolio-product` para la sección "Premium Tool Request Automation" (Power Automate). Revisar si aplica replicarlo en `portfolio-bi` si esa sección existe ahí también.
